...
struct Music_iOS_App_Section_Quiz_One: View {
    ...
    // If you want to allow a "Done" button to pop back:
            ...
            if showResult {
                VStack(spacing: 20) {
                    ...
                    HStack(spacing: 20) {
                        ...
                        // Optional: A "Done" button to pop back to the parent
                        Button("Done") {
    ...
    func checkAnswer() {
        ...
        if currentQuestionIndex < questions.count - 1 {
            ...
            // Update DB for "Level 1"
            DatabaseManager.shared.updateQuiz(
                levelName: "Level 1",
                passed: userPassed,
                goldStar: userPassed  // Or goldStar: (score == questions.count) for perfect
            )
        }
        selectedAnswer = nil
    }
...
// If you haven't defined Question yet, you can do so here or in a separate file:
struct Question {
    let text: String
    let answers: [String]
...
#Preview {
    Music_iOS_App_Section_Quiz_One()
}
